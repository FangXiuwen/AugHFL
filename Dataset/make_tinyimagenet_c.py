...
class DistortImageFolder(data.Dataset):
    def __init__(self, root, method_dic, corrupt_rate=1, transform=None, target_transform=None,
        ...
        corruption_data_len = int(corrupt_rate * 100000)
        self.idxes = idxes[0:corruption_data_len]
        self.root = root
    ...
    def __getitem__(self, index):
        ...
        method = random.sample(self.method_dic.keys(), 1)[0]
        # for severity in range(1,6):
        severity = np.random.randint(1, 5)
        print(d[method].__name__, severity)
        if self.transform is not None:
            img = self.transform(img)
            if index in self.idxes:
                img = d[method](img, severity)
        if self.target_transform is not None:
            target = self.target_transform(target)

        save_path = '../Dataset/tiny_imagenet/Tiny-Imagenet-C_train/random_noise_' + str(self.corrupt_rate) + '/' + self.idx_to_class[target]
...
corrupt_rate = 0.5
data_root = "../Dataset/tiny_imagenet/train"
# data_root = "../Dataset/tiny_imagenet/val"

distorted_dataset = DistortImageFolder(
    root=data_root,
    method_dic=d, corrupt_rate=corrupt_rate, 
