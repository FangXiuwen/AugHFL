...
Corrupt_rate = 1 #[0, 1, 0.5, 0.2, 0.8]
...
"""Dataset Setting"""
Dataset_Name = 'cifar10' #['cifar10c', 'cifar10']
if Corrupt_rate == 0:
    Dataset_Name = 'cifar10'
    Dataset_Dir = '../Dataset/cifar_10'
else:
    Dataset_Name = 'cifar10c'
    Dataset_Dir = '../Dataset/cifar_10/CIFAR-10-C_train'
...
if __name__ =='__main__':
    mkdirs('./Model_Storage/' + Corruption_Type + '_' + str(Corrupt_rate) + '_' + Pariticpant_Params['loss_funnction'])
    logger = init_logs()
    ...
    for index in range(N_Participants):
        train_dl_local ,test_dl, train_ds_local, test_ds = get_dataloader(dataset=Dataset_Name,datadir=Dataset_Dir,train_bs=TrainBatchSize,test_bs=TestBatchSize,dataidxs=net_dataidx_map[index],
        ...
        torch.save(network.state_dict(), './Model_Storage/' + Corruption_Type + '_' + str(Corrupt_rate)+ '_' + Pariticpant_Params['loss_funnction'] + '/'+netname+'_'+str(index)+'.ckpt')
    ...
    for index in range(N_Participants):
        ...
        network = nn.DataParallel(network, device_ids=device_ids).to(device)
        netname = Nets_Name_List[index]
        network.load_state_dict(torch.load('./Model_Storage/' + Corruption_Type + '_' + str(Corrupt_rate)+ '_' + Pariticpant_Params['loss_funnction'] + '/'+netname+'_'+str(index)+'.ckpt'))
